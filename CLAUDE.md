...
- `config.json` — deadline-types (met defaultWeeks/defaultDays, óf `fixedDate:"YYYY-MM-DD"` voor een vaste kalenderdatum los van de eventdatum, bv. budgetten op 1 augustus — jaarlijks te updaten; event-deadline-verwijzing is dan enkel `{type}`) + event-types + postTypes + rollenlijst. Deadline-type-namen kunnen placeholder `{x}` bevatten ("Plan meeting with {x}"): invulveld per event-type (opgeslagen als `x` op de deadline-entry) en in het Other-formulier; resolutie via `.replace('{x}', d.x||'…')`. Wordt via fetch geladen; embedded DEFAULT_CONFIG in index.html is de fallback voor file:// gebruik. **Bij wijziging: beide synchroon houden.**
...
- **Pending activities / review-flow** (`pendingActs.dl/.comm`): in de dl-planner gaan sinds juli 2026 ÁLLE geïmporteerde activiteiten eerst naar de reviewkaart (bewuste keuze Marti: deadlines checken/aanvullen/schrappen vóór ze in het overzicht komen). Herkende types krijgen "Review deadlines" (vult add-form met type-checklist voorgevinkt; `reviewTypeName` bewaart de originele typenaam) of "Add as-is" (direct met voorgedefinieerde deadlines). Onbekende types: event-type toekennen (Apply), "Set up manually" of verwijderen. Comm-import maakt activiteiten wél direct aan. Zit mee in autosave.
