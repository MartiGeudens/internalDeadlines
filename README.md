...
Changes apply immediately in your session; a banner warns while there are unsaved configuration changes (also on closing the tab). To make them permanent for everyone, **download config.json** and commit it to the repository — the site redeploys automatically.
...
A deadline type name may contain the placeholder **`{x}`** (e.g. "Plan meeting with {x}"): a fill-in field then appears wherever the type is used — per event type in the Admin tab (stored as `x` on the event-type deadline entry) and per activity in the *Other* form.
...
Communication posts work like deadline types: `postTypes` defines the shared list (name, channel, default offset in days, default time), and each event type's `commPlan` references them by id with an optional per-event `daysBefore`/`time` override. A `commPlan` is an ordered **list**, so the same post type may appear several times with different offsets (e.g. reminder stories at 10, 7 and 3 days); in the Admin tab you add posts one by one and they stay sorted from earliest to latest. Each entry can carry an optional **note** (e.g. "info about books & money"): it appears under the post in the overview, gets its own **Note column** in the post schedule Excel, and is put at the top of the description of the calendar entry (so titles stay short). Configs in the old format (inline `name`/`channel` per commPlan entry) are migrated automatically at load time.
